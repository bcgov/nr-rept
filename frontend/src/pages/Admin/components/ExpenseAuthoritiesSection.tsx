...
import {
  useExpenseAuthorityMutations,
  useExpenseAuthoritySearch,
...
import type { ExpenseAuthorityDto } from '@/services/rept/admin/types';

type ExpenseAuthorityRow = IdentifiableContent<{
  name: string;
  status: string;
  record: ExpenseAuthorityDto;
}>;

type ExpenseAuthorityFormState = {
  id?: number;
  name: string;
  active: boolean;
};

const ExpenseAuthoritiesSection: FC = () => {
  ...
  const listQuery = useExpenseAuthoritySearch(criteria);
  const mutations = useExpenseAuthorityMutations(criteria);
  ...
  const [formState, setFormState] = useState<ExpenseAuthorityFormState>({
    id: undefined,
    name: '',
    active: true,
  });
  ...
  const [recordToDelete, setRecordToDelete] = useState<ExpenseAuthorityDto | null>(null);
  ...
  const openEditModal = useCallback((record: ExpenseAuthorityDto) => {
  ...
  const handleSubmit = async () => {
    ...
    const payload = { name: trimmed, active: formState.active };
  ...
  const openDeleteConfirm = useCallback((record: ExpenseAuthorityDto) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      showSuccess('Expense authority removed');
  ...
  const rows = useMemo<ExpenseAuthorityRow[]>(
    () =>
  ...
  const tableHeaders: TableHeaderType<ExpenseAuthorityRow>[] = useMemo(
    () => [
      ...
      },
      {
        ...
        selected: true,
        renderAs: (value) => {
          const record = value as ExpenseAuthorityDto;
          return (
  ...
  return (
    <Stack gap={6} className="admin-section">
      ...
      <Modal
        open={isModalOpen}
        size="sm"
        modalHeading={formState.id ? 'Edit expense authority' : 'Add expense authority'}
        passiveModal
      ...
      <DestructiveModal
        ...
        title="Delete expense authority"
...
export default ExpenseAuthoritiesSection;
